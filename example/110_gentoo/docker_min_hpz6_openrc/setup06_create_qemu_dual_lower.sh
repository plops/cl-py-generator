...
EXT4_LOWER_OUT="${QEMU_DIR}/${EXT4_LOWER_NAME}"
KERNEL_OUT="${QEMU_DIR}/vmlinuz"
INITRAMFS_OUT="${QEMU_DIR}/initramfs_dual_lower_sda1-x86_64.img"
CMDLINE_OUT="${QEMU_DIR}/cmdline_dual_lower.txt"
EXT4_LOWER_GUEST_DEV="${EXT4_LOWER_GUEST_DEV:-/dev/vdb}"
EXT4_LOWER_MOUNT_OPTS="${EXT4_LOWER_MOUNT_OPTS:-ro,dax=always}"
...
fix_artifact_permissions() {
  ...
  chmod 0666 "${IMAGE_PATH}" "${EXT4_LOWER_OUT}" 2>/dev/null || true
}
...
EXT4_SRC="${ARTIFACT_DIR}/gentoo.ext4"
...
cp -av --sparse=always "${EXT4_SRC}" "${EXT4_LOWER_OUT}"
...
cat > "${CMDLINE_OUT}" <<EOF
root=live:/dev/vda1 rd.live.dir=/ rd.live.squashimg=gentoo.squashfs rd.live.overlay.overlayfs=1 rd.live.overlay.lower.ext4dev=${EXT4_LOWER_GUEST_DEV} rd.live.overlay.lower.ext4opts=${EXT4_LOWER_MOUNT_OPTS} rd.luks.uuid=${LUKS_UUID} rd.luks.name=${LUKS_UUID}=${GUEST_MAPPER_NAME} console=tty0 console=ttyS0,115200
EOF
...
log "ext4 lower: ${EXT4_LOWER_OUT}"
log "guest lower device: ${EXT4_LOWER_GUEST_DEV}"
log "guest lower mount opts: ${EXT4_LOWER_MOUNT_OPTS}"
...
log "note: actual DAX/XIP still depends on guest kernel and block-device support"

...
eclean packages # 1.2G 65 files
eclean distfiles  # 469.5M 52 files
emerge --jobs=6 --load-average=10   --ask --verbose --update --newuse --deep --with-bdeps=y @world 
6.6.47 -> 6.6.52
emerge --depclean
revdep-rebuild
...
/proc/pressure
isolcpu
/sys/kernel/kheaders.tar.gz
bootconfig.rst
resctrl AMD Qos
wchan
amd rapl events
cpu mitigations
worker queue power efficient (not per cpu)
gcc-plugins.rst
...
make modules_install
make install
cp /boot/vmlinuz /boot/vmlinuz-6.6.52-gentoo-x86_64

emacs /boot/grub/grub.cfg

menuentry 'Gentoo GNU/Linux 20241009 6.6.52 ram squash persist crypt ssd ' --class gentoo --class gnu-linux --class gnu --class os $menuentry_id_option 'gnulinux-simple-80b66b33-ce31-4a54-9adc-b6c72fe3a826' {
	load_video
	if [ "x$grub_platform" = xefi ]; then
		set gfxpayload=keep
	fi
	...
	insmod fat
	search --no-floppy --fs-uuid --set=root F63D-5318
	echo	'Loading Linux 6.6.52-gentoo-x86_64 ...'
# the kernel and initramfs is loaded from nvme0n1p3 (unencrypted)
# the initramfs asks for password and gets the squashfs from nvme0n1p4 (encrypted)
	linux	/vmlinuz-6.6.52-gentoo-x86_64 root=/dev/nvme0n1p3 init=/init mitigations=off
	initrd	/initramfs20241009_squash_crypt-6.6.52-gentoo-x86_64.img
}

menuentry 'Gentoo GNU/Linux 6.6.52 from disk' --class gentoo --class gnu-linux --class gnu --class os $menuentry_id_option 'gnulinux-simple-80b66b33-ce31-4a54-9adc-b6c72fe3a826' {
	load_video
	if [ "x$grub_platform" = xefi ]; then
		set gfxpayload=keep
	fi
	...
	insmod fat
	search --no-floppy --fs-uuid --set=root F63D-5318
	echo	'Loading Linux 6.6.52-gentoo ...'
	linux	/vmlinuz-6.6.52-gentoo-x86_64 root=UUID=80b66b33-ce31-4a54-9adc-b6c72fe3a826 ro  
}

cryptsetup luksOpen /dev/nvme0n1p4 p4
mount /dev/mapper/p4 /mnt4

export TODAY=20241009
export INDIR=/
export OUTFILE=/mnt4/gentoo_$TODAY.squashfs
rm $OUTFILE
time \
...
lib/modules/6.6.30-gentoo-x86_64 \
lib/modules/6.6.38-gentoo-x86_64 \
lib/modules/6.6.47-gentoo-x86_64 \
usr/lib/modules/6.3.12-gentoo-x86_64 \
usr/lib/modules/6.6.12-gentoo-x86_64 \
usr/lib/modules/6.6.17-gentoo-x86_64 \
usr/lib/modules/6.6.30-gentoo-x86_64 \
usr/lib/modules/6.6.38-gentoo-x86_64 \
usr/lib/modules/6.6.47-gentoo-x86_64 \
usr/src/linux* \
var/cache/binpkgs/* \
var/cache/distfiles/* \
gentoo*squashfs \
...
- 5Mb bigger than before (compressed)

Filesystem size 1777865.98 Kbytes (1736.20 Mbytes)
        30.72% of uncompressed filesystem size (5787613.74 Kbytes)
...
dracut \
  -m " kernel-modules base rootfs-block crypt dm " \
  --filesystems " squashfs vfat overlay " \
  --kver=6.6.52-gentoo-x86_64 \
  --force \
  "/boot/initramfs"$TODAY"_squash_crypt-6.6.52-gentoo-x86_64.img"
...
make modules_install
make install
cp /boot/vmlinuz /boot/vmlinuz-6.6.58-gentoo-r1-x86_64

emacs /boot/grub/grub.cfg

menuentry 'Gentoo GNU/Linux 20241112 6.6.58 ram squash persist crypt ssd ' --class gentoo --class gnu-linux --class gnu --class os $menuentry_id_option 'gnulinux-simple-80b66b33-ce31-4a54-9adc-b6c72fe3a826' {
	load_video
	if [ "x$grub_platform" = xefi ]; then
		set gfxpayload=keep
	fi
	...
	insmod fat
	search --no-floppy --fs-uuid --set=root F63D-5318
	echo	'Loading Linux 6.6.58-gentoo-r1-x86_64 ...'
# the kernel and initramfs is loaded from nvme0n1p3 (unencrypted)
# the initramfs asks for password and gets the squashfs from nvme0n1p4 (encrypted)
	linux	/vmlinuz-6.6.58-gentoo-r1-x86_64 root=/dev/nvme0n1p3 init=/init mitigations=off
	initrd	/initramfs20241112_squash_crypt-6.6.58-gentoo-r1-x86_64.img
}

menuentry 'Gentoo GNU/Linux 6.6.58 from disk' --class gentoo --class gnu-linux --class gnu --class os $menuentry_id_option 'gnulinux-simple-80b66b33-ce31-4a54-9adc-b6c72fe3a826' {
	load_video
	if [ "x$grub_platform" = xefi ]; then
		set gfxpayload=keep
	fi
	...
	insmod fat
	search --no-floppy --fs-uuid --set=root F63D-5318
	echo	'Loading Linux 6.6.58-gentoo ...'
	linux	/vmlinuz-6.6.58-gentoo-r1-x86_64 root=UUID=80b66b33-ce31-4a54-9adc-b6c72fe3a826 ro  
}

cryptsetup luksOpen /dev/nvme0n1p4 p4
mount /dev/mapper/p4 /mnt4

export TODAY=20241112
export INDIR=/
export OUTFILE=/mnt4/gentoo_$TODAY.squashfs
rm $OUTFILE
time \
...
- 200Mb bigger than before (compressed with 1 instead of 6), uncompressed 70MB more

Filesystem size 1967989.72 Kbytes (1921.86 Mbytes)
        33.60% of uncompressed filesystem size (5857524.18 Kbytes

39s # 30s faster than before (1m4s)
...
cp ../linux-6.6.58-gentoo-r1/.config .
...
cp /boot/vmlinuz /boot/vmlinuz-6.6.62-gentoo-x86_64

emacs /mnt2/boot/grub2/grub.cfg

menuentry 'Gentoo GNU/Linux 20241201 6.6.62 ram squash persist crypt ssd ' --class gentoo --class gnu-linux --class gnu --class os $menuentry_id_option 'gnulinux-simple-80b66b33-ce31-4a54-9adc-b6c72fe3a826' {
	load_video
	if [ "x$grub_platform" = xefi ]; then
		set gfxpayload=keep
	fi
...
# the kernel and initramfs is loaded from nvme0n1p3 (unencrypted)
# the initramfs asks for password and gets the squashfs from nvme0n1p4 (encrypted)
	linux	/vmlinuz-6.6.62-gentoo-x86_64 root=/dev/nvme0n1p3 init=/init mitigations=off
	initrd	/initramfs20241201_squash_crypt-6.6.62-gentoo-x86_64.img
}

menuentry 'Gentoo GNU/Linux 6.6.62 from disk' --class gentoo --class gnu-linux --class gnu --class os $menuentry_id_option 'gnulinux-simple-80b66b33-ce31-4a54-9adc-b6c72fe3a826' {
	load_video
	if [ "x$grub_platform" = xefi ]; then
		set gfxpayload=keep
	fi
	...
	insmod fat
	search --no-floppy --fs-uuid --set=root F63D-5318
	echo	'Loading Linux 6.6.62-gentoo ...'
...
```
eselect kernel set linux-6.12.16-gentoo
cd /usr/src/linux
...
lib/modules/6.6.52-gentoo-x86_64 \
lib/modules/6.6.58-gentoo-r1-x86_64 \
lib/modules/6.6.62-gentoo-x86_64 \
usr/lib/modules/6.3.12-gentoo-x86_64 \
usr/lib/modules/6.6.12-gentoo-x86_64 \
usr/lib/modules/6.6.17-gentoo-x86_64 \
usr/lib/modules/6.6.30-gentoo-x86_64 \
...
usr/lib/modules/6.6.52-gentoo-x86_64 \
usr/lib/modules/6.6.58-gentoo-r1-x86_64 \
usr/lib/modules/6.6.62-gentoo-x86_64 \
usr/src/linux* \
var/cache/binpkgs/* \
var/cache/distfiles/* \
gentoo*squashfs \
...
# 80MB more (compressed)
# 430MB more (uncompressed)
Filesystem size 2016627.26 Kbytes (1969.36 Mbytes)
        31.20% of uncompressed filesystem size (6462759.07 Kbytes)
...
dracut \
  ...
  --force \
  "/boot/initramfs"$TODAY"_squash_crypt-6.12.16-gentoo-x86_64.img"
...
```
mount /dev/nvme0n1p2 /mnt2
cp /boot/vmlinuz /boot/vmlinuz-6.12.16-gentoo-x86_64

emacs /mnt2/boot/grub2/grub.cfg

menuentry 'Gentoo GNU/Linux 20250301 6.12.16 ram squash persist crypt ssd ' --class gentoo --class gnu-linux --class gnu --class os $menuentry_id_option 'gnulinux-simple-80b66b33-ce31-4a54-9adc-b6c72fe3a826' {
	load_video
	if [ "x$grub_platform" = xefi ]; then
		set gfxpayload=keep
	fi
...
# the kernel and initramfs is loaded from nvme0n1p3 (unencrypted)
# the initramfs asks for password and gets the squashfs from nvme0n1p4 (encrypted)
	linux	/vmlinuz-6.12.16-gentoo-x86_64 root=/dev/nvme0n1p3 init=/init mitigations=off
	initrd	/initramfs20250301_squash_crypt-6.12.16-gentoo-x86_64.img
}

menuentry 'Gentoo GNU/Linux 6.12.16 from disk' --class gentoo --class gnu-linux --class gnu --class os $menuentry_id_option 'gnulinux-simple-80b66b33-ce31-4a54-9adc-b6c72fe3a826' {
	load_video
	if [ "x$grub_platform" = xefi ]; then
		set gfxpayload=keep
	fi
	...
	linux	/vmlinuz-6.12.16-gentoo-x86_64 root=UUID=80b66b33-ce31-4a54-9adc-b6c72fe3a826 ro  
}
...
make
cp ryzen_smu.ko /lib/modules/6.12.16-gentoo-x86_64/kernel/
...
menuentry 'grub2 6.12.16 copy squashfs to ram' {
	load_video
	insmod part_gpt
	search --no-floppy --fs-uuid --set=root F63D-5318
	echo	'Loading Linux 6.12.16 ..."
	linux	/vmlinuz-6.12.16-gentoo-x86_64 root=/dev/nvme0n1p3 init=/init mitigations=off
	initrd	/initramfs_squash_crypt-6.12.16-gentoo-x86_64.img
}

menuentry 'grub2 6.12.16 from disk' {
	load_video
	insmod part_gpt
	search --no-floppy --fs-uuid --set=root F63D-5318
	echo	'Loading Linux 6.12.16-gentoo ...'

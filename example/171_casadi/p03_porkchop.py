...
def create_integrator()->object:
    ...
    F=integrator("F", "cvodes", dae, {("t0"):(0), ("tf"):(1), ("reltol"):((1.00e-10)), ("abstol"):((1.00e-12))})
    return F
# ========================================================================
...
def solve_lambert_single(args)->float:
    ...
    guess_vx=(((1.10    ))*(v_earth_mag)*(np.cos(((angle_to_mars)+(((np.pi)/(6)))))))
    guess_vy=(((1.10    ))*(v_earth_mag)*(np.sin(((angle_to_mars)+(((np.pi)/(6)))))))
    opti.set_initial(v0, vertcat(guess_vx, guess_vy))
    opti.solver("ipopt", {("print_time"):(False)}, {("print_level"):(0), ("max_iter"):(1000), ("tol"):((1.00e-6))})
    try:
        sol=opti.solve()
        total_dv=float(sol.value(((dv_dep)+(dv_arr))))
        if ( ((total_dv)<((((50.    ))*(((YR_S)/(AU_KM)))))) ):
...
# - Transferdauer: 100 bis 450 Tage (Hohmann-Transfer: ~259 Tage)
    n_dep=50
    n_tof=50
    t_dep_arr=np.linspace((0.    ), (2.50    ), n_dep)
    tof_days_arr=np.linspace((1.00e+2), (4.50e+2), n_tof)
    tof_yr_arr=((tof_days_arr)/((365.250    )))
    args_list=[]
    for t_dep in t_dep_arr:
        for tof in tof_yr_arr:
            args_list.append((t_dep,tof,))
    n_cpus=mp.cpu_count()
    n_total=((n_dep)*(n_tof))
    print(f"Starte Gitterberechnung: {n_dep}x{n_tof} = {n_total} Punkte auf {n_cpus} Kernen")
    t_start=time.time()
...
# - Der weisse Stern markiert das globale Minimum.
    ...
    cf=ax.contourf(T_DEP, TOF_DAYS, dv_grid, levels=levels, cmap="RdYlGn_r", extend="max")
    cs=ax.contour(T_DEP, TOF_DAYS, dv_grid, levels=levels, colors="black", linewidths=(0.30    ), alpha=(0.50    ))
    cbar=plt.colorbar(cf, ax=ax)
    cbar.set_label("Total $\\Delta v$ [km/s]", fontsize=13)
    if ( np.any(valid_mask) ):
        ax.plot(t_dep_min, tof_min, marker="*", color="white", markersize=20, markeredgecolor="black", markeredgewidth=(1.50    ), zorder=10)
        ax.annotate(f"$\\Delta v_{{min}}$ = {dv_min:.2f} km/s", xy=(t_dep_min,tof_min,), xytext=(((t_dep_min)+((0.150    ))),((tof_min)+(25)),), fontsize=11, fontweight="bold", color="white", arrowprops={("arrowstyle"):("->"), ("color"):("white"), ("lw"):((1.50    ))})
    ax.set_xlabel("Departure date [years from epoch]", fontsize=13)
    ax.set_ylabel("Transfer duration [days]", fontsize=13)
    ax.set_title("Earth $\\rightarrow$ Mars: Pork Chop Diagram (Impulsive $\\Delta v$)", fontsize=15, fontweight="bold")
    ax.grid(True, alpha=(0.30    ), color="white", linestyle="--")
    a_hohmann=((((A_EARTH)+(A_MARS)))/((2.0    )))
    tof_hohmann_yr=((np.pi)*(np.sqrt(((((a_hohmann)**(3)))/(((MU_SUN)/(((4)*(((np.pi)**(2)))))))))))
    tof_hohmann_days=((tof_hohmann_yr)*((365.250    )))
    ax.axhline(y=tof_hohmann_days, color="cyan", linestyle="--", alpha=(0.70    ), linewidth=(1.50    ), label=f"Hohmann ({tof_hohmann_days:.0f} d)")

...
(progn
  ...
  (write-source
   ...
   `(do0
     ...
     (dot ax1 (plot t_vec z_r_vec :label (string "Road Profile (Bump)") :color (string "gray") :linestyle (string "--") :lw 1.5))
     (dot ax1 (plot t_vec zs_passive :label (string "Passive Chassis") :color (string "crimson") :lw 2))
     (dot ax1 (plot t_vec zs_mpc :label (string "Active Chassis (MPC)") :color (string "royalblue") :lw 2))
     (dot ax1 (set_title (string "Chassis Vertical Displacement")))
     (dot ax1 (set_xlabel (string "Time (s)")))
     (dot ax1 (set_ylabel (string "Displacement (m)")))
     (dot ax1 (grid True :alpha 0.3))
     (dot ax1 (legend))

     ;; Plot 2: Chassis Acceleration
     (setf ax2 (aref axes 0 1))
     (dot ax2 (plot t_vec acc_passive :label (string "Passive") :color (string "crimson") :lw 2))
     (dot ax2 (plot t_vec acc_mpc :label (string "Active (MPC)") :color (string "royalblue") :lw 2))
     (dot ax2 (set_title (string "Chassis Vertical Acceleration (Comfort Metric)")))
     (dot ax2 (set_xlabel (string "Time (s)")))
     (dot ax2 (set_ylabel (string "Acceleration (m/s^2)")))
     (dot ax2 (grid True :alpha 0.3))
     (dot ax2 (legend))

     ;; Plot 3: Suspension Deflection (Stroke)
     (setf ax3 (aref axes 1 0))
     (dot ax3 (plot t_vec (aref x_hist_passive 0) :label (string "Passive") :color (string "crimson") :lw 2))
     (dot ax3 (plot t_vec (aref x_hist_mpc 0) :label (string "Active (MPC)") :color (string "royalblue") :lw 2))
     (dot ax3 (axhline :y 0.08 :color (string "black") :linestyle (string ":") :label (string "Stroke Limit (+/- 8cm)")))
     (dot ax3 (axhline :y -0.08 :color (string "black") :linestyle (string ":")))
     (dot ax3 (set_title (string "Suspension Deflection (Travel)")))
     (dot ax3 (set_xlabel (string "Time (s)")))
     (dot ax3 (set_ylabel (string "Deflection (m)")))
     (dot ax3 (grid True :alpha 0.3))
     (dot ax3 (legend))

     ;; Plot 4: Actuator Control Force
     (setf ax4 (aref axes 1 1))
     (dot ax4 (step t_vec u_hist_mpc :label (string "Active Force (MPC)") :color (string "forestgreen") :lw 2))
     (dot ax4 (axhline :y 1500.0 :color (string "black") :linestyle (string ":") :label (string "Actuator Limit (+/- 1500N)")))
     (dot ax4 (axhline :y -1500.0 :color (string "black") :linestyle (string ":")))
     (dot ax4 (set_title (string "Actuator Control Force")))
     (dot ax4 (set_xlabel (string "Time (s)")))
     (dot ax4 (set_ylabel (string "Force (N)")))
     (dot ax4 (grid True :alpha 0.3))
     (dot ax4 (legend))

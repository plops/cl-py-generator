...
from PySide6.QtWidgets import QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QSlider, QLabel, QGridLayout, QPushButton
...
# =========================================================================================
class PendulumMPC:
    def __init__(self, h_mpc = 5.0e-2, N = 20, use_jit = False, use_to_function = False, use_dual_warmstart = False, use_map = False):
        self.opti=Opti()
        self.nx=4
        self.nu=1
        self.N=N
        ...
        den=M_s+m_s*(1.    -(cos_theta*cos_theta))
        F_total=F_+wind_s*cos_theta
        ds=v_
        dv=(F_total+m_s*l_s*omega_*omega_*sin_theta+m_s*9.81    *cos_theta*sin_theta)/den
        dtheta=omega_
        domega=(( -1.    *F_total*cos_theta)-(m_s*l_s*omega_*omega_*sin_theta*cos_theta)-((M_s+m_s)*9.81    *sin_theta))/(l_s*den)
        ...
        if self.use_map:
            ...
            for k in range(self.N):
                ...
                for j in range(1, self.d+1):
                    ...
                    f_eval=self.f_ode(self.Xc[k][j-1], self.U[:,k], vertcat(self.M_p, self.m_p, self.l_p, self.wind_p))
        ...
        if self.use_jit:
            ...
            solver_opts["jit_options"]={("flags"):(["-O3", "-ffast-math", "-march=native"])}
        ...
        self.sol=None
        self.last_X=np.zeros((self.nx,self.N+1,))
        self.last_U=np.zeros((self.nu,self.N,))
        self.last_lam_g=None
        if self.use_map:
            self.last_Xc_var=np.zeros((self.nx*self.d,self.N,))
        if self.use_to_function:
            inputs=[self.current_x, self.target_x, self.M_p, self.m_p, self.l_p, self.wind_p, self.Q_s, self.Q_v, self.Q_theta, self.Q_omega, self.R_F, self.max_pos, self.max_force, self.X]
            if self.use_map:
    ...
    def step(self, state, target_state, params):
        if self.use_to_function:
            inputs=[state, target_state, params["M"], params["m"], params["l"], params["wind"], params["Q_s"], params["Q_v"], params["Q_theta"], params["Q_omega"], params["R_F"], params["max_pos"], params["max_force"]]
            if (self.sol is not None) or (self.last_lam_g is not None):
                ...
                if self.use_map:
                    Xc_guess=np.zeros((self.nx*self.d,self.N,))
            ...
            try:
                ...
                return (u_opt,self.last_X,self.last_U,t_solve,True,)
            except Exception as e:
                t_solve=time.time()-t0
                return (float(U_guess[0]),self.last_X,self.last_U,t_solve,False,)
        else:
            ...
            if success:
                ...
                self.last_X=np.hstack((self.last_X[:,1:],self.last_X[:,-1:],))
                self.last_U=np.append(self.last_U[1:], self.last_U[-1])
                u_opt=self.last_U[0]
            return (float(u_opt),self.last_X,self.last_U,t_solve,success,)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        if "s"!="t_solve":
            self.pred_curves["s"]=ax.plot(pen=pg.mkPen("y", width=2, style=Qt.DashLine))
        ...
        if "theta"!="t_solve":
            self.pred_curves["theta"]=ax.plot(pen=pg.mkPen("y", width=2, style=Qt.DashLine))
        ...
        if "v"!="t_solve":
            self.pred_curves["v"]=ax.plot(pen=pg.mkPen("y", width=2, style=Qt.DashLine))
        ...
        if "omega"!="t_solve":
            self.pred_curves["omega"]=ax.plot(pen=pg.mkPen("y", width=2, style=Qt.DashLine))
        ...
        if "F"!="t_solve":
            self.pred_curves["F"]=ax.plot(pen=pg.mkPen("y", width=2, style=Qt.DashLine))
        ...
        if "t_solve"!="t_solve":
            self.pred_curves["t_solve"]=ax.plot(pen=pg.mkPen("y", width=2, style=Qt.DashLine))
        ...
        def add_slider(layout, name, label, min_val, max_val, default_val, scale, row, tooltip):
            ...
            lbl=QLabel(f"{label}: {init_val:.2f}" if isinstance(scale, float) else f"{label}: {int(init_val)}")
            ...
            slider.valueChanged.connect(lambda : lbl.setText(f"{label}: {slider.value() * scale:.2f}" if isinstance(scale, float) else f"{label}: {int(slider.value() * scale)}"))
            self.sliders[name]=slider
        add_slider(slider_layout, "M", "Wagenmasse [kg]", 1, 50, 10, 0.1    , 0, "Masse des Wagens. Schwerer = Träger gegen Bewegungsänderungen.")
        add_slider(slider_layout, "m", "Pendelmasse [kg]", 1, 20, 1, 0.1    , 1, "Punktmasse des Pendels am Kopfende.")
        add_slider(slider_layout, "l", "Pendellänge [m]", 1, 20, 5, 0.1    , 2, "Abstand vom Wagen zum Pendelschwerpunkt.")
        add_slider(slider_layout, "wind", "Windkraft [N]", -300, 300, 0, 0.1    , 3, "Konstante Störkraft, die horizontal auf das Pendel drückt.")
        add_slider(slider_layout, "Q_s", "Gewicht Position", 0, 200, 10, 1, 4, "Strafe (Penalty) für Abweichung des Wagens von der Ziel-Position.")
        add_slider(slider_layout, "Q_v", "Gewicht Wagengeschw.", 0, 100, 1, 1, 5, "Strafe für hohe Geschwindigkeit des Wagens (verhindert Überschwingen).")
        add_slider(slider_layout, "Q_theta", "Gewicht Pendelwinkel", 0, 500, 100, 1, 6, "Strafe für das Abweichen des Pendels vom instabilen Gleichgewicht (0 rad).")
        add_slider(slider_layout, "Q_omega", "Gewicht Winkelgeschw.", 0, 100, 1, 1, 7, "Strafe für schnelle Rotationen des Pendels.")
        add_slider(slider_layout, "R_F", "Gewicht Kraftaufwand", 1, 200, 10, 1.0e-2, 8, "Kostenfaktor für die Stellkraft F. Zwingt den Solver, Energie zu sparen.")
        add_slider(slider_layout, "target_s", "Ziel-Position [m]", -100, 100, 10, 0.1    , 9, "Soll-Position des Wagens auf der Schiene.")
        add_slider(slider_layout, "max_pos", "Schiene Limit [m]", 10, 200, 50, 0.1    , 10, "Maximaler erlaubter Fahrweg (Constraint). Der Solver darf diesen nie überschreiten.")
        add_slider(slider_layout, "max_force", "Max Kraft [N]", 10, 300, 150, 0.1    , 11, "Stellgrößenbeschränkung für den Aktuator.")
        add_slider(slider_layout, "N", "Knotenpunkte (N)", 1, 500, 20, 1, 12, "Auflösung des Solvers. N=1 bedeutet nur ein Zeitschritt in die Zukunft.")
        add_slider(slider_layout, "h_mpc", "MPC Schritt [ms]", 1, 200, 50, 1, 13, "Dauer eines MPC-Planungsschritts. T_horizon = N * h_mpc.")
        add_slider(slider_layout, "dt_sim", "Simulations-dt [ms]", 1, 100, 33, 1, 14, "Schrittweite der echten Runge-Kutta Physiksimulation. Hat keinen Einfluss auf den Solver.")
        self.timer=QTimer()
        self.timer.timeout.connect(self.update_loop)
        params=self.get_params()
        self.mpc=PendulumMPC(h_mpc=params["h_mpc"], N=params["N"], use_jit=True, use_to_function=True, use_dual_warmstart=True, use_map=True)
        self.reset_sim()
        self.start_sim()
    def start_sim(self):
        self.dt=(self.sliders["dt_sim"].value())/1.0e+3
        self.timer.start(int(self.dt*1.0e+3))
    ...
    def reset_sim(self):
        ...
        if not hasattr(self, "mpc") or self.mpc.N!=new_N or self.mpc.h_mpc!=new_h:
            print("N oder h_mpc haben sich geaendert. Kompiliere und generiere JIT MPC neu...")
            self.mpc=PendulumMPC(h_mpc=new_h, N=new_N, use_jit=True, use_to_function=True, use_dual_warmstart=True, use_map=True)
        else:
            print("Parameter N und h_mpc unveraendert. Verwende bereits kompilierten JIT MPC Solver wieder.")
        self.pendulum_widget.update_state(self.state, 0.    , 0.    , params["l"], params["max_pos"])
    ...
    def get_params(self):
        return dict(M=(self.sliders["M"].value())/1.0e+1, m=(self.sliders["m"].value())/1.0e+1, l=(self.sliders["l"].value())/1.0e+1, wind=(self.sliders["wind"].value())/1.0e+1, Q_s=float(self.sliders["Q_s"].value()), Q_v=float(self.sliders["Q_v"].value()), Q_theta=float(self.sliders["Q_theta"].value()), Q_omega=float(self.sliders["Q_omega"].value()), R_F=(self.sliders["R_F"].value())/1.0e+2, max_pos=(self.sliders["max_pos"].value())/1.0e+1, max_force=(self.sliders["max_force"].value())/1.0e+1, h_mpc=(self.sliders["h_mpc"].value())/1.0e+3, N=int(self.sliders["N"].value()), dt_sim=(self.sliders["dt_sim"].value())/1.0e+3)
    def update_loop(self):
        ...
        self.dt=params["dt_sim"]
        self.timer.setInterval(int(self.dt*1.0e+3))
        def f_real(st):
            s_st=st[0]
            v_st=st[1]
            theta_st=st[2]
            ...
            l_val=params["l"]
            ds=v_st
            dv=(F_tot+params["m"]*l_val*omega_st*omega_st*sin_t+params["m"]*9.81    *cos_t*sin_t)/den
            dtheta=omega_st
            domega=(( -1.    *F_tot*cos_t)-(params["m"]*l_val*omega_st*omega_st*sin_t*cos_t)-((params["M"]+params["m"])*9.81    *sin_t))/(l_val*den)
        ...
        self.pendulum_widget.update_state(self.state, F_motor, wind_force, params["l"], params["max_pos"])

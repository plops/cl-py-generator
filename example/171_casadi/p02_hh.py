...
def simulate_hh(E: float, y0_list: list, t_max: float = 1.0e+3, n_steps: int = 100000)->list:
    x=SX.sym("x")
    y=SX.sym("y")
    px=SX.sym("px")
    py=SX.sym("py")
    ...
    lambda_val=1.    
    V=0.5    *((x**2)+(y**2))+lambda_val*(((x**2)*y)-((y**3)/3.    ))
    T_kin=0.5    *((px**2)+(py**2))
    H=T_kin+V
...
ax_ph=axs[1,1]
E_low=8.333e-2
y0_low=np.linspace(-0.25    , 0.42    , 12).tolist()
results_low=simulate_hh(E_low, y0_low, t_max=5.0e+3, n_steps=500000)
E_high=0.15    
y0_high=np.linspace(-0.35    , 0.55    , 12).tolist()
results_high=simulate_hh(E_high, y0_high, t_max=5.0e+3, n_steps=500000)
x_g=np.linspace(-1.2    , 1.2    , 200)
y_g=np.linspace(-1.2    , 1.2    , 200)
X, Y=np.meshgrid(x_g, y_g)
Z=0.5    *((X**2)+(Y**2))+(((X**2)*Y)-((Y**3)/3.    ))
for res in results_low:
    ax_ol.plot(res["x"], res["y"], alpha=0.5    )
    ax_pl.scatter(res["y_cross"], res["py_cross"], s=0.6    , alpha=0.8    )
ax_ol.contour(X, Y, Z, levels=[E_low], colors="red", linestyles="dashed")
for res in results_high:
    ax_oh.plot(res["x"], res["y"], alpha=0.5    )
    ax_ph.scatter(res["y_cross"], res["py_cross"], s=0.6    , alpha=0.8    )

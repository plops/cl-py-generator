...
void Display::small_text(std::string text, float x, float y, uint8_t h,
                         uint8_t s, uint8_t v) {
  text(text, pax_font_sky, x, y, h, s, v);
}
void Display::large_text(std::string text, float x, float y, uint8_t h,
                         uint8_t s, uint8_t v) {
  text(text, pax_font_saira_condensed, x, y, h, s, v);
}
void Display::text(std::string text, pax_font_t *font, float x, float y,
                   uint8_t h, uint8_t s, uint8_t v) {
  auto text_ = text.c_str();

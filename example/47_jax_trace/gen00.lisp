		  ...
		  ,(let ((l `((none 0.0)
			...
			(setf eye (np.tile (np.array (list 0 2.0 -3.5)
			      ...
			      rd (vn (+ (* (aref p ":" 0 None)
					...
					(* d w)))))
		       (do0
			(comments "naive path tracer, 25W light source")
			(setf LIGHT_POWER (np.array (list 25 25 25)))
			(setf LIGHT_AREA 1.0)
			(def emittedRadiance (p ro)
			  (return (/ LIGHT_POWER
				     (* np.pi LIGHT_AREA))))
			(def trace (ro rd depth)
			  (setf p (intersect ro rd)
				n (calcNormal p)
				radiance (emittedRadiance p ro)
				)
			  (when (< depth 3)
			    (setf rd2 (sampleUniformHemisphere n)
				  Li (trace p rd2 (+ depth 1))
				  )
			    (incf radiance (* (brdf p rd rd2)
					      Li
					      (np.dot rd n))))
			  (return radiance)))
		       )))))))

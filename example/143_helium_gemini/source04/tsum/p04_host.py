...
MODEL_OPTIONS=["gemini-2.5-flash-preview-09-2025| input-price: 0.3 output-price: 2.5 max-context-length: 128_000", "gemini-2.5-pro| input-price: 1.25 output-price: 10 max-context-length: 200_000", "gemini-2.5-flash-lite-preview-09-2025| input-price: 0.1 output-price: 0.4 max-context-length: 128_000"]
...
def get_transcript(url, identifier):
    ...
    try:
        ...
        cmds_en=["uvx", "yt-dlp", "--skip-download", "--write-auto-subs", "--write-subs", "--cookies-from-browser", "firefox", "--sub-langs", "en-orig", "-o", sub_file, "--", youtube_id]
        ...
        if ( os.path.exists(sub_file_en) ):
            ...
            logger.info("English subtitles not found. Trying to download subtitles in other languages.")
            for lang in ["en", "de", "zh", "iw", "lv", "fr", "pl", "ja"]:
                cmds_any=["uvx", "yt-dlp", "--skip-download", "--write-auto-subs", "--write-subs", "--cookies-from-browser", "firefox", "--sub-langs", lang, "-o", sub_file, "--", youtube_id]
                ...
                if ( subtitle_files ):
                    ...
                    logger.info(f"Parse transcript from {sub_file_to_parse} out of the subtitle files: {subtitle_files}")
                    break
...
def get(request: Request):
    ...
    nav=Nav(Ul(Li(Strong("Transcript Summarizer"))), Ul(Li(A("Map", href="https://rocketrecap.com/exports/index.html")), Li(A("Privacy Policy", href="https://rocketrecap.com/exports/privacy.html")), Li(A("Demo Video", href="https://www.youtube.com/watch?v=ttuDW1YrkpU")), Li(A("Documentation", href="https://github.com/plops/gemini-competition/blob/main/README.md"))))
    chrome_ext_promo=Article(H2("Summarize Faster with the Chrome Extension!", style="margin-bottom: 0.5rem;"), P("Tired of copying and pasting links? Install the official RocketRecap browser extension to summarize any YouTube video, article, or selected text with a single click."), H3("How to Use:"), Ol(Li("Keep a tab open with this page (rocketrecap.com)."), Li("On any YouTube video or article, click the extension icon to send it for summarization."), Li("Return to the rocketrecap.com tab and refresh the page. Your new summary will appear at the top.")), A(Img(src="https://rocketrecap.com/exports/cws.png", alt="Available on the Chrome Web Store", style="height: 58px; width: auto;"), href="https://chrome.google.com/webstore/detail/lbkchnlbfpibkooidbngaiilmegfkbej", target="_blank", title="Available on the Chrome Web Store"), style="margin-top: 2rem; margin-bottom: 2rem; border-color: var(--pico-primary-border);")
    ...
    model=Div(Select(*selector, style="width: 100%;", name="model"), style="display: flex; align-items: center; width: 100%;")
    form=Form(Fieldset(Div(Textarea(placeholder="Link to youtube video (e.g. https://youtube.com/watch?v=j9fzsGuTTJA)", name="original_source_link"), transcript, model, Button("Summarize Transcript"), style="display: flex; flex-direction:column;")), data_hx_post="/process_transcript", data_hx_swap="afterbegin", data_hx_target="#summary-list")
    summaries_to_show=summaries(order_by="identifier DESC", limit=3)
    summary_list_container=Div(*summaries_to_show, id="summary-list")
    return Title("Video Transcript Summarizer"), Meta(name="description", content="Get AI-powered summaries of YouTube videos and websites. Paste a link or transcript to receive a concise summary with timestamps."), Main(nav, NotStr(documentation_html), chrome_ext_promo, form, summary_list_container, Script("""function copyPreContent(elementId) {
...
def generation_preview(identifier):
    ...
    text="Generating ..."
    trigger="every 1s"
    price_input={("gemini-2.5-flash-preview-09-2025"):((0.30    )), ("gemini-2.5-pro"):((1.250    )), ("gemini-2.5-flash-lite-preview-09-2025"):((0.10    ))}
    price_output={("gemini-2.5-flash-preview-09-2025"):((2.50    )), ("gemini-2.5-pro"):(10), ("gemini-2.5-flash-lite-preview-09-2025"):((0.40    ))}
    try:
...
Output tokens: {output_tokens}"""
        ...
        summary_details=Div(P(B("identifier:"), Span(f"{s.identifier}")), P(B("model:"), Span(f"{s.model}")), A(f"{s.original_source_link}", target="_blank", href=f"{s.original_source_link}", id=f"source-link-{identifier}"), cls="summary-details")
        ...
        card_content=[Header(H4(A(f"{s.original_source_link}", target="_blank", href=f"{s.original_source_link}")), P(f"ID: {s.identifier} | Model: {s.model.split('|')[0]}", style="font-size: 0.9em; color: var(--pico-secondary-foreground); margin-bottom: 0;")), Div(NotStr(html)), Footer(hidden_pre_for_copy, Button("Copy Summary", onclick=f"copyPreContent('pre-{identifier}')", class="outline"))]
        if ( ((trigger)==("")) ):
            return Article(*card_content, id=sid)
        else:
            return Article(*card_content, id=sid, data_hx_post=f"/generations/{identifier}", data_hx_trigger=trigger, data_hx_swap="outerHTML")
    except Exception as e:
...
def generate_and_save(identifier: int):
    ...
    try:
        ...
        summaries.update(pk_values=identifier, summary_done=False, summary=((summaries[identifier].summary)+("\nError1234: resource exhausted")), summary_timestamp_end=datetime.datetime.now().isoformat(), timestamps="", timestamps_timestamp_start=datetime.datetime.now().isoformat())
        return 
    except Exception as e:
    ...
    try:
        ...
        if ( transcript_text ):
            ...
            embedding_result=genai.embed_content(model="models/embedding-001", content=transcript_text, task_type="clustering")
    ...
    try:
        ...
        if ( summary_text ):
            ...
            embedding_result=genai.embed_content(model="models/embedding-001", content=summary_text, task_type="clustering")
            vector_blob=np.array(embedding_result["embedding"], dtype=np.float32).tobytes()

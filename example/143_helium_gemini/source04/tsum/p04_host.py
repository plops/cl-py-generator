...
# Configure Uvicorn's logging to use UTC timestamps.
# Modify the format for the default and access loggers.
LOGGING_CONFIG["formatters"]["default","fmt"]="%(asctime)s.%(msecs)03dZ [%(name)s] %(levelprefix)s %(message)s"
LOGGING_CONFIG["formatters"]["default","datefmt"]="%Y-%m-%dT%H:%M:%S"
LOGGING_CONFIG["formatters"]["access","fmt"]="%(asctime)s.%(msecs)03dZ [%(name)s] %(levelprefix)s %(client_addr)s - "%(request_line)s" %(status_code)s"
LOGGING_CONFIG["formatters"]["access","datefmt"]="%Y-%m-%dT%H:%M:%S"
# Monkey-patch Uvicorn's formatters to use time.gmtime for UTC conversion.
uvicorn.logging.DefaultFormatter.converter=time.gmtime
uvicorn.logging.AccessFormatter.converter=time.gmtime
# Get a logger for application-specific messages.
logger=logging.getLogger(__name__)
logger.info("Uvicorn logging configured to use UTC timestamps.")
...
def render(summary: Summary):
    ...
    if ( summary.timestamps_done ):
        ...
        return Div(NotStr(markdown.markdown(summary.summary)), id=sid, data_hx_post=f"/generations/{identifier}", data_hx_trigger=("") if (summary.timestamps_done) else (""), data_hx_swap="outerHTML")
    else:
        return Div(NotStr(markdown.markdown(summary.summary)), id=sid, data_hx_post=f"/generations/{identifier}", data_hx_trigger="", data_hx_swap="outerHTML")
...
def get(request: Request):
    ...
    nav=Nav(Ul(Li(H1("RocketRecap Content Summarizer"))), Ul(Li(A("Map", href="https://rocketrecap.com/exports/index.html")), Li(A("Extension", href="https://rocketrecap.com/exports/extension.html")), Li(A("Privacy Policy", href="https://rocketrecap.com/exports/privacy.html")), Li(A("Demo Video", href="https://www.youtube.com/watch?v=ttuDW1YrkpU")), Li(A("Documentation", href="https://github.com/plops/gemini-competition/blob/main/README.md")), Li(A("Log out", href="/logout"))))
    ...
    selector=[Option(opt, value=opt, label=opt.split("|")[0]) for opt in MODEL_OPTIONS]
...
AI-generated summary created with {s.model.split('|')[0]} for free via RocketRecap-dot-com. (Input: {input_tokens:,} tokens, Output: {output_tokens:,} tokens, Est. cost: {cost_str})."""
        ...
        summary_details=Div(P(B("identifier:"), Span(f"{s.identifier}")), P(B("model:"), Span(f"{s.model}")), A(f"{s.original_source_link}", target="_blank", href=f"{s.original_source_link}", id=f"source-link-{identifier}"), P(B("generate_abstract:"), Span(f"{s.generate_abstract}")), cls="summary-details")

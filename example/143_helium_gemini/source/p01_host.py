...
def get():
    transcript = Textarea(
        placeholder="Paste YouTube transcript here", name="transcript"
    )
    model = Select(
    ...
    form = Form(
        Group(transcript, model, Button("Send Transcript")),
    ...
    return Title("Video Transcript Summarizer"), Main(
        H1("Summarizer Demo"), form, gen_list, cls="container"
    )
...
def post(transcript: str, model: str):
    ...
    if (20_000) < (len(words)):
        return Div(
            "Error: Transcript exceeds 20,000 words. Please shorten it.", id="summary"
        )
...
def generate_and_save(prompt, id, model):
    ...
    response = m.generate_content(
        f"I don't want to watch the video. Create a self-contained bullet list summary: {prompt}",
        stream=True,
    )

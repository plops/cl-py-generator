...
def get(request: Request):
    ...
    model=Div(Select(Option("gemini-1.5-flash-002"), Option("gemini-1.5-pro-002"), Option("gemini-1.5-pro-exp-0827"), Option("gemini-1.5-pro-exp-0801"), Option("gemini-1.5-flash-exp-0827"), Option("gemini-1.5-flash-8b-exp-0924"), Option("gemini-1.5-flash-latest"), Option("gemma-2-2b-it"), Option("gemma-2-9b-it"), Option("gemma-2-27b-it"), Option("gemini-1.5-flash"), Option("gemini-1.5-pro"), Option("gemini-1.0-pro"), style="width: 100%;", name="model"), style="display: flex; align-items: center; width: 100%;")
    form=Form(Group(Div(transcript, Textarea(placeholder="(Optional) Add Link to original source", name="originalSourceLink"), model, Div(Label("Output Language", _for="outputLanguage"), Select(Option("en"), Option("de"), Option("fr"), Option("ch"), Option("nl"), Option("pt"), Option("cz"), Option("it"), Option("jp"), Option("ar"), style="width: 100%;", name="outputLanguage", id="outputLanguage"), style="display: flex; align-items: center; width: 100%;"), Div(Input(type="checkbox", id="includeComments", name="includeComments", checked=False), Label("Include User Comments", _for="includeComments"), style="display: flex; align-items: center; width: 100%;"), Div(Input(type="checkbox", id="includeTimestamps", name="includeTimestamps", checked=True), Label("Include Timestamps", _for="includeTimestamps"), style="display: flex; align-items: center; width: 100%;"), Div(Input(type="checkbox", id="includeGlossary", name="includeGlossary", checked=False), Label("Include Glossary", _for="includeGlossary"), style="display: flex; align-items: center; width: 100%;"), Button("Summarize Transcript"), style="display: flex; flex-direction:column;")), hx_post="/process_transcript", hx_swap="afterbegin", target_id="gen-list")
...
Output tokens: {output_tokens}"""
        ...
        title=f"identifier: {s.identifier} model: {s.model} host: {s.host} originalSourceLink: {s.originalsourcelink} includeComments: {s.includecomments} includeTimestamps: {s.includetimestamps} includeGlossary: {s.includeglossary} outputLanguage: {s.outputlanguage} cost: {s.cost}"

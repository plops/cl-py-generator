...
def get(request: Request):
    ...
    form=Form(Group(Div(transcript, Textarea(placeholder="(Optional) Add Link to original source", name="originalSourceLink"), model, Div(Label("Output Language", _for="outputLanguage"), Select(Option("en"), Option("de"), Option("fr"), Option("ch"), Option("nl"), Option("pt"), Option("cz"), Option("it"), Option("jp"), Option("ar"), style="width: 100%;", name="outputLanguage"), style="display: flex; align-items: center; width: 100%;"), Div(Input(type="checkbox", id="includeComments", name="includeComments"), Label("Include User Comments", _for="includeComments"), style="display: flex; align-items: center; width: 100%;"), Div(Input(type="checkbox", id="includeTimestamps", name="includeTimestamps"), Label("Include Timestamps", _for="includeTimestamps"), style="display: flex; align-items: center; width: 100%;"), Div(Input(type="checkbox", id="includeGlossary", name="includeGlossary"), Label("Include Glossary", _for="includeGlossary"), style="display: flex; align-items: center; width: 100%;"), Button("Summarize Transcript"), style="display: flex; flex-direction:column;")), hx_post="/process_transcript", hx_swap="afterbegin", target_id="gen-list")
...
def generate_and_save(identifier: int):
    ...
    try:
        response=m.generate_content(f"""Below, I will provide input for an example video (comprising of title, description, and transcript, in this order) and the corresponding summary I expect. Afterward, I will provide a new transcript that I want you to summarize in the same format. 

...
def get(request: Request):
    ...
    summary_list=Ul(*summaries(order_by="id DESC"), id="summaries")
...
def generate_and_save(id: int):
    ...
    if ( ((FinishReason.SAFETY)==(response.candidates[0].safety_ratings)) ):
        print("stopped because of safety")
    for chunk in response:
        try:
            print(f"add text to id={id}: {chunk.text}")
            summaries.update(pk_values=id, summary=((summaries[id].summary)+(chunk.text)))
        except ValueError:
            
            print("Value Error")
    if ( response._done ):
        summaries.update(pk_values=id, summary_done=True, summary_input_tokens=response.usage_metadata.prompt_token_count, summary_output_tokens=response.usage_metadata.candidates_token_count, summary_timestamp_end=datetime.datetime.now().isoformat())
    else:

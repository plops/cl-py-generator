...
### `(setf x 0.3333333333333333d0
           y 123456.78901234567d0
           z 1.0d-20
           w 1.0d30)`
Tests that float literals are emitted with enough digits to read back as the very same number.
...
### `(print (== 0.3333333333333333d0 (/ 1 3)))`
Verifies that an emitted double float literal is bit-identical to the value Python computes.
...
**Generated Python (after formatting):**
```python
print(0.3333333333333333 == (1 / 3))

```

### `(list (? (> x 0) (for-generator (x xs) x)))`
Tests the two-argument ternary as the filter of a comprehension (the else branch is omitted).
...
### `(/ a b c)`
Tests division with more than two arguments (left associative, like Common Lisp and Python).
...
### `(// a b c)`
Tests floor division with more than two arguments.
